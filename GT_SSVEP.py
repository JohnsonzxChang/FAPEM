...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_size, heads, max_len):
        super(MultiHeadAttention, self).__init__()
        # 计算在时间维度上的多头注意力机制
        self.positional_encoding = PositionalEncoding(embed_size, max_len=max_len)
...
class Model(nn.Module):
    def __init__(self, num_node, input_dim, hidden_dim, output_dim, embed_dim, cheb_k, horizon, num_layers, heads, A, kernel_size, max_len):
        ...
        self.MultiHeadAttention = MultiHeadAttention(embed_size=hidden_dim, heads=heads, max_len=max_len)
        # predict
    ...
    def forward(self, x):
        ...
        # print(f"sa dim:{SAtt.shape},, ta dim:{TAtt.shape}")
        out = SAtt + TAtt
        out = self.end_conv(out.permute(0, 3, 2, 1)).squeeze(1)  # [B, 1, N, T] -> [B, N, T]
...
if __name__ == "__main__":
    ...
    A = torch.tensor(np.random.random((9, 9)))
    model = Model(num_node=9, input_dim=3, hidden_dim=8, output_dim=32, embed_dim=64, 
                  cheb_k=3, horizon=50, num_layers=4, heads=4, A=A, kernel_size=3, max_len=50)
